...
class StoryList {
  ...
  async addStory(user, newStory) {
    ...
    const response = await axios({
      url: `${BASE_URL}/stories`,
      method: "POST",
      data: { token: userToken, story: newStory },
    });
    return new Story(response.data.story);
  }

  /** Deletes story data from API 
   * - user - the current instance of User who will delete the story
   * - obj of story
  */

  async delStory(user, story) {
    const userToken = user.loginToken;
    console.dir(story);
    const response = await axios({
    ...
    user.ownStories = user.ownStories.filter(val => val.storyId !== story.storyId);
  }
}
...
class User {
  ...
  /** Handle favorites. Allows user to favorite and unfavorite stories */
